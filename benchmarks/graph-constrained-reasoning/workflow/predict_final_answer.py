...
def make_prediction(data, args, processed_list, input_builder, model, node_labels):
    ...
    result.update({k: data[k] for k in ['id', 'question', 'seed_nodes', 'seed_nodes_id', 'answer_node', 'answer_node_id', 'sparql_query', 'all_answers', 'answer_subgraph', 'full_subgraph', 'n_hops', 'graph_template', 'answer_triples', 'redundant']})
    result.update({
        "graphrag_subgraph": data['predicted_paths'],
...
def main(args, LLM):
    ...
    if args.add_path:
        if args.use_all:
            ...
            prediction_suffix = f"NEW_add_path_{path_name}"
            paths_datasets = []
            with open(args.reasoning_path, "r") as f:
                for line in f:
                    paths_datasets.append(json.loads(line))
            if len(paths_datasets) == 1:
                paths_datasets = paths_datasets[0]
    ...
    for data in tqdm(dataset):
        res = make_prediction(data, args, processed_list, input_builder, model, node_labels)
        if res is not None:
            if args.debug:
                print(json.dumps(res))
            fout.write(json.dumps(res) + "\n")
