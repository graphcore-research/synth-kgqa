...
os.environ["WANDB_PROJECT"] = "graph-constrained-reasoning_finetuning"
...
class ScriptArguments:
    ...
    kg_path: str = field(metadata={"help": "Path to KG."})
...
def train():
    ...
    node_labels = np.load(
        osp.join(script_args.kg_path, "node_labels.npy"),
        allow_pickle=True,
    )
    ...
    dataset_list = []
    ds = datasets.load_from_disk(script_args.data_path_list[0])
    for _ in range(int(training_args.num_train_epochs)):
        df = ds.to_pandas().groupby(["id", "seed_answ_pair"], as_index=False).agg(np.random.choice)
        new_ds = datasets.Dataset.from_pandas(df)
        new_ds = new_ds.shuffle()
        dataset_list.append(new_ds)
    ...
    def input_formatter(example):
        chunks = []
        for i in range(len(example["seed_nodes_id"])):
            if np.random.rand() < 0.5:
                question = example["paraphrased_question"][i]
            else:
                question = example["question"][i]
            start_node = node_labels[example["seed_nodes_id"][i]]
            # anser_node = node_labels[example["answer_node_id"][i]]
