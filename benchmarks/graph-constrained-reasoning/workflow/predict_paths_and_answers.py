...
def prediction(data, id, processed_list, input_builder, model, node_labels, relation_labels, data_trie=None):
    ...
    if data_trie:
        assert data["question"] == data_trie["question"]
        input_query, ground_paths, trie = data_trie["input_query"], data_trie["ground_truth_paths"], data_trie["trie"] 
    else:
        gt_graph = [[node_labels[h], relation_labels[r], node_labels[t]] for h,r,t in data["answer_subgraph"]]
        graph = [[node_labels[h], relation_labels[r], node_labels[t]] for h,r,t in zip(data["h_id_list"], data["r_id_list"], data["t_id_list"])]

        input_query, ground_paths, trie = input_builder.process_input({"question": question, "q_entity": [node_labels[x] for x in data["seed_nodes_id"]], "a_entity": answer, "graph": graph, "gt_graph": gt_graph})
...
def main(args, LLM):
    ...
    if args.n > 1:
        ...
        for i, data in enumerate(tqdm(dataset)):
            res = prediction(data, i, processed_list, input_builder, model, node_labels, relation_labels)
            if res is not None:
                if args.debug:
                    print(json.dumps(res))
                fout.write(json.dumps(res) + "\n")
