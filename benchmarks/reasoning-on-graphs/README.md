...
```bash
python src/align_kg/build_align_qa_dataset.py --kg-path <path to KG> --data_path <path to qa folder> --split <file name of output of compute_negihs_and_sp.py>
```

Add flag `--use_sp` to build training data using shortest paths between seed and answer node, instead of paths in ground-truth answer subgraph.
