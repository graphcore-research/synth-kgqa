...
def gen_prediction(args):
    ...
    def prepare_dataset(sample, node_labels, relation_labels):
        ...
        paths_list = []
        for seed_answ_pair, p_list in sample["seed_answer_paths"].items():
            for p in p_list:
                p_lab = [
                    [
                        node_labels[trip[0]],
    ...
    dataset = [prepare_dataset(x, node_labels, relation_labels) for x in dataset]
