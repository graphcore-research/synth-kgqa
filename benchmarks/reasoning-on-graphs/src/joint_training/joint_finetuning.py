...
os.environ["WANDB_PROJECT"] = "reasoning-on-graph_finetuning"
...
def train():
    ...
    # Load datasets
    train_dataset = load_multiple_datasets(script_args.data_path_list, int(training_args.num_train_epochs), shuffle=True)
    ...
    sft_cfg = SFTConfig(
        **{k: v for k, v in training_args.to_dict().items() if k != "model_max_length"},
